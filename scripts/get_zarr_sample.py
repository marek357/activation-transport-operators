...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Create a sample from a zarr archive file")
    parser.add_argument(
        "--source",
        "-s",
        type=str,
    ...
    root_group.attrs["max_seq_len"] = int(max(source_z["attention_mask"][:n_samples].sum(axis=1)))
